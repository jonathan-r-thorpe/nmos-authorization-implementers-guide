...
3. If Node has not yet registered to the Authorization Server, do dynamic client registration to the Authorization Server.
4. Node starts the client credentials flow to fetch bearer token.
5. Node refreshes bearer token at least 15 seconds before token expiry (i.e. half-life of the token).
6. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTI3OTAzMDAyOSwxNjgwNDAyNDE2LC0xND
