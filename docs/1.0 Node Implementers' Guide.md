...
1. Node fetches Authorization Server Metadata to obtain Authorization Server supported endpoints and features.
2. If Node has not yet registered to the Authorization Server, do dynamic client registration to the Authorization Server.
3. Node starts the client credentials flow to fetch bearer token.
4. Node attempts to refresh bearer token at least 15 seconds before token expiry (i.e the half-life of the token).
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE2OTk0NTgzNzksLTE0NzAzMzAwNzQsMj
ExNTc1Nzk2LC0xODM5MzM4NDQxLC01NTgzNTI1NjIsMjA4ODgy
MzExNyw2MTIxNjY5ODUsODkzNzc5MTEwLDE4NDQzMjE0NjFdfQ
==
-->