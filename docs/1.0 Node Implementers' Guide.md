...
IS-10 is based on the OAuth2 recommendation, it is used for protecting the NMOS APIs, which allowing NMOS Node and Registry to give the limited access for the third party application. Third application including Broadcast Controller System, which queries NMOS Registry via IS-04 for all the registered Nodes information and making IS-05 Nodes connection. NMOS Nodes are also acted as the third party for the NMO Registry, as requires to perform IS-04 node registration.  In this document, we are focused on the Node implemenation.
...
_Maybe include this as part of the Node to Registry communications?_
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjgzNDY3NjgxLC01NTgzNTI1NjIsMjA4OD
gyMzExNyw2MTIxNjY5ODUsODkzNzc5MTEwLDE4NDQzMjE0NjFd
fQ==
-->