...
_Maybe include this as part of the Node to Registry communications?
 ...
 1. Node fetches Authorization Server Metadata to obtain Authorization
    Server supported endpoints and features. If Node has not yet
    registered to the Authorization Server, do dynamic client
    registration to the Authorization Server. Node starts the client
    credentials flow to fetch bearer token. Node attempts to refresh
    bearer token at least 15 seconds before token expiry (i.e the
    half-life of the token).
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE5NTY1ODcxNDYsLTE0NzAzMzAwNzQsMj
