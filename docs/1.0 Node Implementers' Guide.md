...
3. Node [fetches Authorization Server Metadata](https://specs.amwa.tv/is-10/branches/v1.0-dev/docs/3.0._Discovery.html#authorization-server-metadata-endpoint) to obtain Authorization Server supported endpoints and features.
4. If Node has not yet registered to the Authorization Server, do [dynamic client registration](https://specs.amwa.tv/is-10/branches/v1.0-dev/docs/4.2._Behaviour_-_Clients.html#client-registration) to the Authorization Server.
5. Node starts the client credentials flow to fetch bearer token.
6. Node refreshes bearer token at least 15 seconds before token expiry (i.e. half-life of the token).
7. Node switches to next available Authorization Server, if it is unable to commuincate to connected Authorization Server.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTE3MDgyNzk2MiwtODAxNTIyNjcsMTY4MD
QwMjQxNiwtMTQ3MDMzMDA3NCwyMTE1NzU3OTYsLTE4MzkzMzg0
NDEsLTU1ODM1MjU2MiwyMDg4ODIzMTE3LDYxMjE2Njk4NSw4OT
M3NzkxMTAsMTg0NDMyMTQ2MV19
-->