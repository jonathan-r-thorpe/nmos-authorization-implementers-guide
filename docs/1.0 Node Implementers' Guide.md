...
IS-10 is based on the OAuth2 recommendation, it is used for protecting the NMOS APIs, which allowing NMOS Node and Registry to give the limited access for the third party application. Third application including Broadcast Controller System, which queries Registry via IS-04 for Nodes information and making IS-05 connection on Nodes. Nodes are also acted as the third party for the Registry, as requires to perform IS-04 node registration.  In this document, we are focused on the Node implemenation.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTU1ODM1MjU2MiwyMDg4ODIzMTE3LDYxMj
E2Njk4NSw4OTM3NzkxMTAsMTg0NDMyMTQ2MV19
-->