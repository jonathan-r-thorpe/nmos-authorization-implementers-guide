...
Access token is built of three segements, each are encoded separately using [Base64url Encoding](https://en.wikipedia.org/wiki/Base64#URL_applications "Base64"), and concatenated using periods to produce the JWT:
...
Although JWT.io, has a number of third party libraries which supporting the JWT validation between step 1 and step 3. A custom validation is still be required for checking the NMOS private claims on step 4.

<!--stackedit_data:
eyJoaXN0b3J5IjpbMTMwODYxODk3MCwtODQ2MTU0NDU3XX0=
-->