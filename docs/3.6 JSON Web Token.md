...
There are number of steps consider while validating the access token. 

1.  Check that the JWT is well formed , such as JWT contains three segments, hedaer, payload and signature, separated by two period ('.') characters
...
First verify the signature by re-constructing the signawith the issuer Public Key, to ensure token has not been tampered. Then  can be veified  To verify the access token, 
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEwODM5ODI3OTVdfQ==
-->