...
Payload contains the token details, such as when the token is becoming acative, when it is expire, who the token expected for, and some standard common claims. IS-10 has also introduced some NMOS related private claims, which are related to the NMOS scopes.  Each NMOS private claim has the Read and Write permission.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTg5NjU3NDU3M119
-->